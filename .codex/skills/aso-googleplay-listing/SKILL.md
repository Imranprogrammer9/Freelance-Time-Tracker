...
4. **Brand tone** — Ask the user (wait for their answer):
...
If the developer already ran Call /kit-start-setup and answered the "what is your
...
**Confirm which groups to target** — Ask the user (wait for their answer), allowing
multiple selections: one option per group — label = group name, description = the
keywords inside it. The developer ticks the groups that match how they want to be
found. Phrase the question so the strongest group reads as the recommended default.
