...
- **Auth is fully configured** for the selected provider — ask the developer
...
First ask the developer (wait for their answer) whether the app needs auth at
all. If they pick "no auth", set `AUTH_ENABLED = false` in `KitConfig` and stop
here.

Otherwise ask the developer (wait for their answer) which provider — present
these two options:
...
Ask the developer (wait for their answer) which sign-in methods the auth screen
should offer:
...
(if you use Play App Signing) from Play Console: **Protected with Play → Play
...
way — from your release keystore, or (if you use Play App Signing) from Play
Console: **Protected with Play → Play Store protection → Manage Play app
signing** (or search `app signing`). Mention this as a release-time task only —
do NOT walk through it now.
