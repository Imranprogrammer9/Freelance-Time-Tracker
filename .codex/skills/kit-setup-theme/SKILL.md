...
- **Both already set to non-default values** — Ask the user (wait for their answer):
...
default pack swappable in one line. Ask the user (wait for their answer) which
pack the app should ship with. Include the preview URL in each option's
description so the developer can browse the style before picking:
