...
copy customised by Call /kit-start-setup). This skill REPLACES that with a
...
Before starting:
- App must compile — Call /kit-compile-app. Stop if it does not.
...
If the brand tone is not obvious from the existing copy, Ask the user (wait for
their answer):
- **Minimal**
- **Friendly**
- **Bold**
...
Call /kit-compile-app. Fix any errors. Then summarise:
- Number of new screens + their names.
- New DataStore keys added to `SettingsRepository`.
- "Call /kit-run-app to see the flow end-to-end."
