...
Before Step 1, call **TaskCreate** with these tasks:

- Step 1 — Verify analytics is wired
- Step 2 — Read context (changelog + app shape)
...
Mark each `in_progress` when entering and `completed` when done.
...
AskUserQuestion with three or four options tailored to **what you read in
...
ship — no new analytics this release" — mark remaining tasks `[skipped]`.
...
Show the list to the developer and AskUserQuestion: "Approve this set? Or
edit?". If they want changes, iterate once. Do not propose more than 5 — funnel
clarity drops sharply past that.
