...
4. **Service-account JSON → RevenueCat** (the actual Play ↔ RevenueCat connection;
   ...
   **done for the developer by the kit** using the `gcloud` CLI, OR walked manually
   in the web console. Ask (AskUserQuestion):
   ...
   **4B — Grant it access in Play Console** (both paths do this). Show verbatim,
   then wait for "done":
   > **Play Console:**
   ...
   **4C — Upload to RevenueCat** (both paths do this). Show verbatim, then wait for
   "done":
   > **RevenueCat:**
