...
Show this verbatim, then AskUserQuestion ("Start these now" / "Later"):
...
Ask (**AskUserQuestion** — don't write a paragraph; the options carry the why):
...
If **Yes** → run `/kit-save-to-github` inline (it handles GitHub login, creates the
